...
public class StudentService implements IStudentService {
  ...
  @Override
  public void update(Long studentId, StudentRequestDTO studentRequestDTO)
    ...
    if (studentRequestDTO.getParentId().equals(actualStudent.getParent().getId())) {
      Parent optParent =
          this.parentRepository
              .findById(studentRequestDTO.getParentId())
              .orElseThrow(() -> new ResourceNotFoundException(studentRequestDTO.getParentId()));
    ...
    if (studentRequestDTO.getTeacherId().equals(actualStudent.getTeacher().getId())) {
      Teacher optTeacher =
          this.teacherRepository
              .findById(studentRequestDTO.getTeacherId())
              .orElseThrow(() -> new ResourceNotFoundException(studentRequestDTO.getTeacherId()));
            ...
            .orElseThrow(
                () ->
                    new ResourceNotFoundException(
                        "ParentId ".concat(String.valueOf(studentRequestDTO.getParentId()))));
    Teacher optTeacher =
        this.teacherRepository
            .findById(studentRequestDTO.getTeacherId())
            .orElseThrow(
                () ->
                    new ResourceNotFoundException(
                        "TeacherId ".concat(String.valueOf(studentRequestDTO.getTeacherId()))));
  ...
  public void deleteById(Long idStudent) throws ResourceNotFoundException {
    Student actualStudent =
        studentRepository
            .findById(idStudent)
            .orElseThrow(() -> new ResourceNotFoundException(idStudent));
